...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($usuario) {
        ...
        $_SESSION['username'] = $usuario['username'] ?? $username;
        $_SESSION['email'] = $usuario['email'] ?? '';
        $_SESSION['foto']  = $usuario['foto']  ?? '';

        if (empty($_SESSION['email']) || empty($_SESSION['foto'])) {
            try {
                mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);

                $stmt = $conn->prepare('SELECT email, foto FROM usuarios WHERE username = ? LIMIT 1');
                $stmt->bind_param('s', $_SESSION['username']);
                $stmt->execute();
                if ($row = $stmt->get_result()->fetch_assoc()) {
                    if (empty($_SESSION['email']) && !empty($row['email'])) {
                        $_SESSION['email'] = $row['email'];
                    }
                    if (empty($_SESSION['foto']) && !empty($row['foto'])) {
                        $_SESSION['foto'] = $row['foto'];
                    }
                }
                $stmt->close();
            } catch (Throwable $e) {
                error_log('Login: no se pudo completar email/foto desde BD: '.$e->getMessage());
            }
        }

        if (empty($_SESSION['email'])) $_SESSION['email'] = 'privado@gefautomocion.com';
        if (empty($_SESSION['foto']))  $_SESSION['foto']  = 'assets/img/account/user.jpg';
